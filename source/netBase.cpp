...
int netBase::clientCommand(char command, int index) {
    ...
    if (index == TO_SERVER) {
        if (command == FACE_COMMAND) {
            ...
            int cur=0;
            while(1){
                cur+=receive_data(TO_SERVER, &tmpImage[Id]+cur, fsize);
                if(cur  >=fsize)break;

            }
            cout <<"get imagedata from "<<Id<<" size ="<<cur<<endl; 

            FILE *fp;
            char tmpchar[15];
            sprintf(tmpchar,"data/%d.png",Id);
            fp = fopen(tmpchar, "wb");
            if (fp != NULL) {
                tmpImage[Id] = (char *) malloc((int) fsize);
                ...
                sys->faceImage[Id] = sys->draw->pngTexture(tmpstring);
            }   else{
    ...
    int netBase::serverCommand(char command, int index) {
        ...
        if (command == FACE_COMMAND) {
            ...
            int cur=0;
            while(1){
                cur+=receive_data(index, &tmpImage[Id]+cur, fsize);
                if(cur  >=fsize)break;
