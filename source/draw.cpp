...
void Draw::routine() {
...

	{
		...
		//sys->model[0].Draw(1.0,1.0,1.0,0.5);

		/*ATTACK1*/
		if (sys->player[sys->myID].attflag == 1
				&& sys->player[sys->myID].attpatern == 1) {
			drawAttack(
					sys->player[sys->myID].x
							+ (sin(sys->player[sys->myID].dir) * 5),
					sys->player[sys->myID].z
							+ (cos(sys->player[sys->myID].dir) * 5));
		}

		/*ATTACK2*/
		if (sys->player[sys->myID].attflag == 1
				&& sys->player[sys->myID].attpatern == 2) {
			if (sys->player[sys->myID].dir >= 0
					&& sys->player[sys->myID].dir < 1.59
					|| sys->player[sys->myID].dir <= 0
							&& sys->player[sys->myID].dir > -1.59) {
				drawAttack2(
						sys->player[sys->myID].x
								+ sin(sys->player[sys->myID].dir) * 6,
						sys->player[sys->myID].z
								+ cos(sys->player[sys->myID].dir) * 5);
			}

			else if (sys->player[sys->myID].dir >= 1.59
					&& sys->player[sys->myID].dir < 1.8
					|| sys->player[sys->myID].dir <= -1.59
							&& sys->player[sys->myID].dir > -1.8) {
				drawAttack2(
						sys->player[sys->myID].x
								+ sin(sys->player[sys->myID].dir) * 8,
						sys->player[sys->myID].z
								+ cos(sys->player[sys->myID].dir) * 16);
				cout << "attack2" << endl;
			}

			else if (sys->player[sys->myID].dir >= 1.8
					&& sys->player[sys->myID].dir < 3.4
					|| sys->player[sys->myID].dir <= -1.8
							&& sys->player[sys->myID].dir > -3.4) {
				drawAttack2(
						sys->player[sys->myID].x
								+ sin(sys->player[sys->myID].dir) * 8,
						sys->player[sys->myID].z
								+ cos(sys->player[sys->myID].dir) * 11);
			}

			else if (sys->player[sys->myID].dir >= 3.4
					&& sys->player[sys->myID].dir < 4.53
					|| sys->player[sys->myID].dir <= -3.4
							&& sys->player[sys->myID].dir > -4.53) {
				drawAttack2(
						sys->player[sys->myID].x
			...
			else if (sys->player[sys->myID].dir >= 4.53
					&& sys->player[sys->myID].dir < 4.74
					|| sys->player[sys->myID].dir <= -4.53
							&& sys->player[sys->myID].dir > -4.74) {
				drawAttack2(
						sys->player[sys->myID].x
			...
			else if (sys->player[sys->myID].dir >= 4.74
					&& sys->player[sys->myID].dir <= 6.03
					|| sys->player[sys->myID].dir <= -4.74
							&& sys->player[sys->myID].dir >= -6.03) {
				drawAttack2(
						sys->player[sys->myID].x
...
void Draw::drawSky(int x, int y) {
	...
	GLfloat vertices[4][3] = { { 26, 3.0, 80 }, { -5, 3.0, 80 },
			{ -5, 3.0, -2.0 }, { 26, 3.0, -2.0 }, };
	glBegin(GL_POLYGON);
	glNormal3f(vertices[0][0], vertices[0][1], vertices[0][2]);
	glTexCoord2i(0, 0);
	glVertex3fv(vertices[0]);
...
void Draw::drawAttack(double x, double z) {
	...
	glTranslatef(-(1 + x), 0, -(1 + z));
	GLfloat vertices[4][3] = { { 2 + x, 0.3, z + 2 }, { 0 + x, 0.3, z + 2 }, { 0
			+ x, 0.3, z }, { 2 + x, 0.3, z }, };
	glBegin(GL_POLYGON);
	glNormal3f(sys->player[sys->myID].xd, sys->player[sys->myID].yd, 0);
	glTexCoord2i(0, 0);
	...
	GLfloat vertices2[4][3] = { { 2 + x, 1.5, z }, { 0 + x, 1.5, z }, { 0 + x,
			0.3, z }, { 2 + x, 0.3, z }, };
	glBegin(GL_POLYGON);
	glNormal3f(sys->player[sys->myID].xd, sys->player[sys->myID].yd, 0);
	glTexCoord2i(0, 0);
	...
	glBindTexture(GL_TEXTURE_2D, *texHandle[5]);
	GLfloat vertices3[4][3] = { { 2 + x, 1.5, 2 + z }, { 0 + x, 1.5, 2 + z }, {
			0 + x, 0.3, 2 + z }, { 2 + x, 0.3, 2 + z }, };
	glBegin(GL_POLYGON);
	glNormal3f(sys->player[sys->myID].xd, sys->player[sys->myID].yd, 0);
	glTexCoord2i(0, 0);
	glVertex3fv(vertices3[0]);
	...
	glBindTexture(GL_TEXTURE_2D, *texHandle[5]);
	GLfloat vertices4[4][3] = { { x, 1.5, 2 + z }, { x, 1.5, z }, { x, 0.3, z },
			{ x, 0.3, 2 + z }, };
	glBegin(GL_POLYGON);
	glNormal3f(sys->player[sys->myID].xd, sys->player[sys->myID].yd, 0);
	glTexCoord2i(0, 0);
	...
	glBindTexture(GL_TEXTURE_2D, *texHandle[5]);
	GLfloat vertices5[4][3] = { { 2 + x, 1.5, 2 + z }, { 2 + x, 1.5, z }, { 2
			+ x, 0.3, z }, { 2 + x, 0.3, 2 + z }, };
	glBegin(GL_POLYGON);
	glNormal3f(sys->player[sys->myID].xd, sys->player[sys->myID].yd, 0);
	glTexCoord2i(0, 0);
